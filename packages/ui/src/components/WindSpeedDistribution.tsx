...
export function WindSpeedDistribution({
  data,
  width,
  height = 300,
  className,
  ...
  const chartData = useMemo(() => {
    ...
    return data.bins.map(
      (b: { binStart: number; binEnd: number; frequency: number; weibullFrequency: number }) => ({
  ...
  if (loading) {
    return (
      <div
        ...
        aria-busy="true"
        aria-label="Loading wind speed distribution"
      />
    );
  }

  if (chartData.length === 0 || data.weibullK === null || data.weibullC === null) {
    return (
  ...
  return (
    <div
      className={className}
      role="img"
      aria-label="Wind speed frequency distribution with Weibull curve fit"
    >
      ...
      <ResponsiveContainer width={width ?? '100%'} height={height}>
        <ComposedChart data={chartData} margin={{ top: 5, right: 20, bottom: 5, left: 0 }}>
          ...
          <XAxis
            dataKey="name"
            tick={{ fontSize: 10 }}
            label={{ value: 'm/s', position: 'insideBottomRight', offset: -5 } as never}
          />
          <YAxis
            tick={{ fontSize: 11 }}
            label={{ value: '%', angle: -90, position: 'insideLeft' } as never}
          />
          <Tooltip
