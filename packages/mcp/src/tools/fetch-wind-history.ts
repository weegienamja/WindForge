...
const inputSchema = z
  .object({
    ...
    years: z
      ...
      .optional()
      .describe(
        'Number of years of monthly history to fetch, ending at the previous calendar year. Default 10.',
      ),
  })
  .strict();
...
export const fetchWindHistoryTool: ToolDefinition<typeof inputSchema> = {
  ...
  inputSchema,
  handler: async (input) => {
    const result = await fetchMonthlyWindHistory({ lat: input.lat, lng: input.lng }, input.years);
    if (!result.ok) {
